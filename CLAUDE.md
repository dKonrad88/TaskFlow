...
- **Adicionei o modelo `seed_snack_protein`** ("Snack Protein", tipo produto) no `PROJ_MODELOS` (topo do array, ~26272): 8 fases → grupos por setor → **122 tarefas** (111 da planilha + 11 sugeridas que o Diego escolheu num checklist interativo), com `prio:'alta'` nos portões (pesquisa de mercado, formulação, sensorial, rotulagem Anvisa, preço, teste piloto, liberar p/ venda, viabilidade). Distribuição por etapa: 8/14/15/16/8/22/24/15. Gerado por `scratchpad/gen_snack.js` (v2) → re-injetado por brace-matching no primeiro elemento do array. As 2 tarefas da "Maria" entraram em Expedição (mapa de armazenamento) e Comercial (ficha comercial). `criarProjetoDeTemplate` cria fielmente fases+grupos(setor)+tarefas e copia a `descricao`. Testado no preview: instanciou 8 fases / 18 grupos / 111 tarefas.

...
- **Helper `_cpEscComprador(nome)`** = `_cpCategoria(nome) ∈ {Matéria-prima, Embalagens}`. **`_cpItens()` filtra na ORIGEM** → cobre de uma vez Painel/Planejamento/Estoque/Insumos (todos usam `_cpItens()`). As views por LINHA (Histórico `_cpFiltradas`, Fornecedores `_cpFornAgg`) ganharam `if(!_cpEscComprador(l.item[1])) return`. **`_CP_CATS` reduzido a `['Matéria-prima','Embalagens']`** (chips de filtro). `_cpSimCriarTarefa` passou a usar `_cpIdx()[cod]` (não o `_cpItens()` agora filtrado). A **Necessidade da semana** já era escopada (seed de MP/embalagem).

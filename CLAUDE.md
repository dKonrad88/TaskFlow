...
- 🐞 **FIX: data de carga com ANO PARCIAL bagunçava o "dura até" das outras cargas + ano de 2 dígitos** (`8d2854b0`, bug do Diego). Ao digitar o ano de uma carga (ex.: 2027), o `<input type=date>` passa por estados inválidos (0002/0020/0202); a carga "chega no ano 2" **reordenava a fila FEFO** e mudava o "dura até" das OUTRAS cargas ao vivo. Fix: **`_cpSimFefoData.temData` agora ignora datas com ano < 2000** (ainda sendo digitadas) → o "dura até" fica estável. Bônus: **`_cpSimEntregaCommit` (novo, no `onchange` da data) normaliza ano de 2 dígitos** — digitar **26/27** no ano vira **2026/2027** (0026→2026). Testado Amendoim (4 cargas): Carga 1 "dura até 01/12/26" tanto com Carga 3 em 2027 quanto em 0002; `0026-10-10`→`2026-10-10`. ⚠️ Só as datas das ENTREGAS/cargas do cronograma — as outras datas (período custom, modo "durar até") seguem com ano de 4 dígitos (re-renderizam no blur, sem o bug); dá p/ estender se o Diego pedir.

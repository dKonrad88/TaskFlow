...
> ▶ **RETOMAR:** Processos (avulso + embutido, **gerando tarefas de verdade no Meu Dia**) + simplificação do projeto + RH Organograma estão no ar e funcionando. **Próximos passos naturais:** (a) **selo/agrupamento visual** das tarefas de processo ("Processo: X" na fase, como o mockup) — mexe no `_tarefaLinha`, fazer com o Diego olhando; (b) decidir a **data do embutido** (hoje-espalhada × seguir a esteira do projeto); (c) decidir se vai além na estrutura (setor como atributo puro); (d) **faxina + cabeçalho de decisão** da ficha de Insumos; (e) RH: quebrar organograma em 2 blocos se o texto ficar pequeno. ⚠️ Nova máquina: `git pull` no começo + (se editar DADOS) ☁️ Baixar da nuvem antes. `git fetch` religioso antes de editar/commitar (rede da Empresa derruba 443, retry passa).

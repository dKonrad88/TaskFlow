...
### 2026-08-31 (e) — PC da Empresa — Simulador de compra → CRIAR TAREFA + ficha (Preço de custeio numa linha só)
Autônomo. **Commits `8f227a4a` (custeio) + `b134669e` (tarefa), pushados; `main == origin`; working tree limpo.** Verificado por DOM (:8899): tarefa criada com os campos certos, botão presente, 0 erros de JS.
- ⭐ **Botão "Criar tarefa desta compra"** no resultado do **Simulador de Compra** (`_cpSimResult`, CTA azul no fim) — pedido do Diego: "simulei a compra, agora criar uma tarefa disso pra não esquecer". Nova **`_cpSimCriarTarefa(cod)`** (~9310, logo após `_cpSimResult`): cria tarefa REAL no HUB (`tasks.push`+`saveTask_db`) com **título** "Comprar N UM de \<item\>" e **descrição** = resumo da compra (qtd+t, item/cód, preço, custo total, cobertura após, entregas, teto de custeio). `prio:'alta'`, `date:today`, `executor`+`solicitante`=**`CURRENT_USER_ID`** (em produção = o comprador logado), `origem:'compra'`+`cpCod`. Cai no **Meu Dia / Minhas Tarefas**. ⚠️ **AÇÃO explícita (nunca no render)**. Toast com **"Desfazer"** (`deleteTask_db`) se foi sem querer. Testado: Amendoim estoque p/ 90d → "Comprar 30.531 KG de Amendoim", desc com 5 linhas, alta, hoje; cleanup ok. Sem dedup (botão manual = intenção explícita; se clicar 2× cria 2, dá p/ excluir).

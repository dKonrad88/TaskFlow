...
- **Nova `_cpPrecoMedLine(o,opts)`** (logo após `_cpComprasLine`, ~8831): sobre a série MENSAL (`_cpItemSerie`, campo `precoKg`). Baseline = **o MÊS ANTERIOR com compra** (Diego corrigiu 27/07: *"a bolinha sempre seria em cima da compra anterior"* — a 1ª versão usava média móvel de 3 e deixava jul/25=5,05 VERDE mesmo tendo SUBIDO vs mai/25=4,50; "não fechava"). Faixa neutra **±3%**: subiu >3% = vermelho, caiu >3% = verde, senão azul. Força `lineCor:'var(--blue-mid)'` e `cor:s=>s._cor`; `opts` mescla p/ servir a aba Gráfico (`onClick/selYm`) e o Resumo (`h:50,fixedH:72…`).

...
- **Embutir em projeto** (`_projAddProcesso`, `_projAddProcessoAbrir`): botão **"Processo"** no cabeçalho do detalhe da fase (Painel) → seletor dos elegíveis (ambos/projeto) → os passos viram **tarefas da fase** (ordenadas pela esteira via `_ppOrdemNovaNaFase`, vinculadas por `processoId`/`processoRunId`/`processoNome`, setor sugerido em `procSetor`, `projectProGrupoId:null`).
- ⚠️ **PENDÊNCIAS anotadas:** (1) o **avulso** é checklist próprio e **NÃO gera tarefas no Meu Dia** (só o **embutido** gera tarefas de verdade) — unificar depois; (2) as tarefas embutidas aparecem **soltas na fase, sem selo visual** de "processo" — NÃO pus selo no `_tarefaLinha` de propósito (grid de 7 colunas fixas é delicado e não dá p/ validar visual nesta máquina); o mockup mostrava um bloco "Processo: X" — fazer com o Diego olhando.

**⭐ PROJETO SIMPLIFICADO — Fase → Tarefa vira o PADRÃO** (parte do `3a81a761`).
- Diego achou confuso "estrutura + etapas + tarefas" (3 níveis). **Decisão (versão SEGURA, não a refatoração profunda):** o detalhe da fase agora abre em **Lista plana (Fase → Tarefa)** por padrão; **"Por setor" virou opcional** (toggle). Flip em `pp-plan-lista` (`renderProjectProVisao` ~28255: default `!=='0'` = Lista). **Setores NÃO foram removidos** — seguem no toggle "Por setor" e na aba Fases; só deixaram de ser nível obrigatório na frente. A refatoração "setor = atributo puro / etiqueta" (a que eu recomendei) **NÃO foi feita** (risco alto sem validação) — é o próximo passo se o Diego quiser ir além.
...
> ▶ **RETOMAR:** Processos + simplificação do projeto + RH Organograma estão no ar e funcionando. **Próximos passos naturais:** (a) **selo visual** das tarefas de processo na fase + agrupar como "Processo: X" (o mockup era assim); (b) fazer o **avulso gerar tarefas de verdade** no Meu Dia (unificar com o embutido); (c) decidir se vai além na estrutura (setor como atributo puro); (d) **faxina + cabeçalho de decisão** da ficha de Insumos; (e) RH: quebrar organograma em 2 blocos se o texto ficar pequeno. ⚠️ Nova máquina: `git pull` no começo + (se editar DADOS) ☁️ Baixar da nuvem antes. `git fetch` religioso antes de editar/commitar (rede da Empresa derruba 443, retry passa).

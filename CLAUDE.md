...
- **Tela (2 colunas):** lista de grupos (busca por nome/mensagem, prévia, não lidas, "@" quando te mencionaram) · conversa (divisória
  ...
  **popover** (`_chatMembrosPop`) aberto clicando no nome do grupo no cabeçalho ou em ⋮ › Membros (adicionar, tornar admin, remover,
  ...
  sem ser um card gigante"): o `.chat-wrap` não tem FUNDO — lista e conversa ficam **direto no fundo da página**, só com um **contorno leve** em volta (3ª rodada: `border:1px var(--border)` + raio 14px, sem preenchimento; o padding passou p/ lista/cabeçalho/feed/campo); 4ª rodada: contorno um pouco mais forte = `color-mix(--text 20%)`. **Saiu o chip "Falando como"** do cabeçalho → `_chatEu()` agora devolve sempre `CURRENT_USER_ID` e limpa o `tf_chat_eu` antigo no carregamento (`_chatTrocarEu` removida); com isso a notificação de ção só aparece se outra aba/pessoa mencionar. Subtítulo virou "Conversas da equipe, organizadas por grupo" e a dica do rodapé tirou "tarefa / decisão" (o Diego disse que num 1º momento não haverá tarefas/decisões — os botões ☑/⚖️ no hover das mensagens e o exemplo do grupo Marca Própria AINDA existem; decidir se saem),
...
- 📌 Lição: `git apply --cached --unidiff-zero` (patch sem contexto) **posiciona errado** quando o arquivo tem outras mudanças acima — gerar o patch com `-U3` e juntar os hunks sem separador.
- ⭐ **2ª rodada (mesmo dia):** o Diego achou a tabela ainda "muita informação, ruim de achar", **sem querer tirar nada**. A tabela foi **girada: cada MÉTRICA virou uma linha** (Valor · Volume · Notas · Itens) e as comparações viraram **colunas lado a lado** — `Base · Atual · Variação · Diferença` | `Ano passado · Este ano · Variação · Diferença` (cabeçalho em 2 níveis: "Períodos comparados" × "Fechamento do ano"). Nada saiu: período e datas foram p/ o **cabeçalho** de cada coluna; unidade (e o ⚠ de unidades mistas) p/ o **rótulo da linha** Volume; "est." virou "2026 projetado*" no cabeçalho + nota de rodapé (que agora também diz "notas e itens não se projetam"). Colunas Variação/Diferença levemente tingidas e divisória antes do bloco do ano. 4 linhas em vez de 8, cada célula numa linha só.
- **Rótulo do preset `ano`** (`_CP_FORN_PER`): "Ano atual" → **"Ano todo"** (pedido do Diego — o recorte é o ano inteiro do ANO escolhido no 2º seletor, não só o corrente). Vale p/ o passo a passo, o card "Alterar comparação" e a aba Itens.
- **"Alterar comparação" deixou de ser card** (`_cpFornCmpQuickHTML`): virou uma **linha discreta** sem fundo — "⇄ Comparando [A][ano] com [B] · ↻ Nova comparação" — com selects pequenos **sem `.modal-input`** (46px) e fundo `var(--bg)` (transparent deixaria a lista do select ilegível no tema escuro). A explicação "atualiza sozinho e organiza Base → Atual" foi p/ o `title` do ícone. E a frase do resultado passou a começar com **"No período selecionado, …"** (pedido do Diego, p/ não confundir com o ano/histórico).
- ⭐ **3ª rodada: DOIS CARDS lado a lado** (pedido do Diego — "são duas análises"): **"Períodos selecionados"** (frase "No período selecionado…" + Base · Atual · Variação · Diferença) e **"Fechamento do ano"** · mantendo o ritmo atual (frase própria "Mantendo o ritmo atual, 2026 deve fechar em R$ X: R$ Y a mais que 2025" + Ano passado · Este ano · Variação · Diferença + a nota da projeção). Mesmas 4 linhas (Valor · Volume · Notas · Itens) nos dois. Grid `auto-fit minmax(min(100%,640px),1fr)` → lado a lado em tela larga, empilha em tela estreita.
- ⚠️ **ARMADILHA DE CSS (vale p/ o HUB todo):** existe uma regra global **`select:not([class]){height:40px !important}`** — `height` inline em select SEM classe é ignorado. Para um select pequeno, dê **qualquer classe** a ele (aqui `cp-cmp-sel`, que nem tem CSS). O mesmo vale p/ `input:not([class])` (40px, sem !important).
- **Cenário de negociação** (`_cpFornSimOutHTML`): cada bloco (Novo volume / Novo valor) ganhou a linha **"Base: X + N%"** — o volume e o valor do período atual de onde o cálculo saiu — e o topo passou a trazer as **datas** do período ("Sobre 2026 (em curso) · 01/01/26 – 11/09/26 · com +15%"). Pedido do Diego: saber de onde veio o número.
- **Lista de Fornecedores ganhou a coluna "Crescimento"** (após "% acum.", ordenável): acumulado **deste ano (01/01 → hoje) × mesmo trecho do ano passado**, por fornecedor (`_cpFornCrescMap`, cache por dia). ⚠️ **Não segue o filtro "Período"** da tela — por isso o cabeçalho diz "2026 × mesmo período 2025". Tooltip na célula com os dois valores; sem compra no trecho do ano passado mostra "novo no ano" (ou "—"). Mesmo escopo da curva ABC (insumos de MP/embalagem).
- **Botão "Gerar PDF" na aba Itens da ficha do fornecedor** (`_cpFornItPDF`, ao lado da lupa): pdfMake, **A4 deitado**, cabeçalho KLAIN, nome do fornecedor, Período A/B com datas, a tabela das 13 colunas (mesmas cores: valor/volume azul×âmbar, preço vermelho×verde) e a **linha de Total** igual ao rodapé da tela. Exporta **o que a tela mostra**: mesma busca, mesma ordenação e, **se houver itens marcados, só os marcados**. Abre na pré-visualização da OS (`_osPdfModal`, que ganhou 3º parâmetro opcional `titulo`) com "Baixar PDF".
- ⭐ **"Gerar PDF" em TODAS as abas da ficha do fornecedor** (pedido do Diego): o botão saiu de perto da lupa da aba Itens e virou **um só, na barra das abas** (ao lado de "Tela cheia"), que gera o PDF da **aba aberta** (`_cpFornPDF` despacha por `_cpFornView`). **Resumo** (`_cpFornResumoPDF`, A4 em pé): os 4 indicadores do topo, volume e valor por mês numa **tabela com barra proporcional** (substitui os 2 gráficos), Indicadores, itens mais comprados, **todas** as OCs em aberto (a tela para em 6) e últimas compras. **Comparar** (`_cpFornCompPDF`): os 2 cards + o cenário de negociação; **sem a comparação montada, só avisa**. **Entradas** (`_cpFornEntrPDF`): as 60 linhas da tela, na ordem da tela, + total. Com itens marcados, todos avisam no topo ("Recorte: só N itens marcados na aba Itens").
  - ⚠️ A conta dos 2 cards e do cenário saiu p/ **`_cpFornCompDados(c)`** e **`_cpFornSimDados(c)`**, e as linhas do card Indicadores p/ **`_cpFornResumoIndRows`** — a TELA e o PDF usam a mesma função (mexeu numa, vale nas duas). Helpers comuns dos PDFs: `_cpPdfPronto`, `_cpFornPdfDoc` (cabeçalho KLAIN + rodapé), `_cpFornPdfTopo`, `_cpPdfSecao`, `_cpFornPdfAbrir`, `_CP_PDF_LAYOUT`, `_cpPdfCorVar/_cpPdfCorPreco/_cpPdfPct`. Verificado no navegador (Pamplastic): os 4 PDFs geram, números da tela iguais aos de antes, 0 erros de console.

### ⭐ 2026-09-06 — Mac — NOVA ABA "Oficina de Ideias" (Gestão): métodos guiados
Espaço p/ qualquer pessoa **planejar/entender/melhorar** com métodos guiados. Fluxo: descreve a situação → recomendador (chips de intenção + sugestão por palavra-chave) → escolhe método → **wizard passo-a-passo** (1 pergunta por vez, respostas anteriores editáveis) → **resumo acionável** (Copiar / Pedir ajuda ao Claude). Arquitetura: **método = config** (`OFC_METODOS`) + motor único (`_ofc*`); adicionar método = adicionar entrada, sem UI nova. v1 com 5 métodos: **SWOT · 5 Porquês · PDCA · Retrospectiva (Manter/Melhorar/Parar) · Prós e Contras**. Dados em `taskflow_oficina` (sincroniza no Supabase como as demais chaves). Wiring: `navSubItem('oficina',…)` na nav Gestão + dispatch no `render()` + título/sub/exclusões no `setTab` + guard do person-view. Módulo inserido antes de `_procVazio`.
- **Pendências/decisões futuras (com o Diego):** (1) absorver ou linkar o app **Metas** (SMART/5W2H hoje é app separado); (2) adicionar **LICOR (Dale Carnegie)** — falta o Diego confirmar as letras/etapas; (3) mais métodos (GUT, Ishikawa, OKR, Esforço×Impacto — este precisa de UI de matriz, não só texto); (4) recomendador "de verdade" por texto livre (hoje é chips + sugestão por keyword + botão Claude); (5) virar as ações do resumo em tarefas do HUB.
- ⚠️ **Heads-up sobre o simulador de compra:** ao dar `git ff` pros teus 98 commits (fornecedor/09-01), notei que o handler `_cpSimEntregaLive` (meu fix do bug "data preenche sozinho", da sessão do Mac 09-06 anterior) **não existe mais** na tua versão — provável que a refatoração do simulador ("resultado em 3 cards / cronograma legível") tenha substituído. Não toquei no simulador (é teu domínio agora); só **confere** se digitar/escolher a data das cargas ainda não trava o ano/perde o foco.
- Git: parti do teu `16b13da`, re-apliquei a Oficina por cima (ff limpo, base linear). Meu trabalho anterior do simulador (ruptura inline, média-3 por meses, cobertura pós-compra, "preço bruto") sobreviveu.

### ⭐⭐ 2026-09-01 (c) — PC da Empresa — FICHA DO FORNECEDOR vira PAINEL de NEGOCIAÇÃO (comparação A×B, ano projetado, simulador) + ORDENAÇÃO em todas as tabelas de Compras
Sessão longa e muito iterativa (o Diego foi refinando por print). **19 commits `29e2a2a3`…`5bc4e202`, todos pushados; `main == origin`; working tree limpo.** Verificado por DOM (:8899) a cada leva — 9 telas de Compras + 4 abas da ficha do fornecedor + ficha do item, 30 renders do simulador (15 fornecedores × bases A/B), **0 erros de console** (aba limpa).
- ⭐ **A ficha do fornecedor virou PAINEL** (espelha a do item): sub-navbar **Resumo · Comparar · Itens · Entradas**, tela cheia, `_cpFornView`. Resumo tem KPIs (Comprado (tudo) com o período · Notas · Itens diferentes · **Ritmo — últ. 3 meses**) + **2 gráficos** (volume/mês e valor/mês).
- ⭐ **COMPARAÇÃO A × B** (`_cpFornCompHTML`): **período A de um lado, B do outro, as diferenças NO MEIO** (Valor · Volume · Itens). Cada lado leva o **próprio seletor** + datas concretas (`01/01/25 – 01/09/25`) + fundo próprio (`CP_BG_A` azulado / `CP_BG_B` cinza) + **divider** separando o quadro do ano dos números do filtro. Nas PONTAS do card, o **quadro do ano**: à esquerda **ano fechado** (fato), à direita **projeção do ano corrente**. 13 presets de período (ano, 4 trimestres, 2 semestres, últ. 3/6/12, **mesmo período do ano passado**, tudo, custom) + seletor de ANO ao lado — lista **plana** de propósito (com optgroup o `<select>` nativo ganhava barra de rolagem).
- ⚠️ **`_cpFornAno(linhas, ano)` — a projeção NÃO é regra de três.** Usa o crescimento já observado (acumulado do ano ÷ mesmo trecho do ano anterior) aplicado ao **ano anterior COMPLETO** → a sazonalidade entra na conta (quem concentra compra no fim do ano seria subestimado por extrapolação linear). Sem ano anterior, cai no linear. Ano < corrente = `fechado:true` (soma real).
- ⚠️ **`_cpFornOrdem(c)` — a variação é SEMPRE cronológica** (do mais antigo p/ o mais recente), não "A vs B". Sem isso, escolher A = ano passado e B = este ano mostrava **queda** num fornecedor que cresceu (bug real que o Diego pegou).
- ⚠️ **"Ritmo — últ. 3 meses" usa JANELA DE CALENDÁRIO**, não os 3 últimos REGISTROS. Com registros, o Dekel Agro comparava "mai–ago/26 × jun/25–abr/26" (4 meses × 11) e dava **+70,9%**; com janela fixa ancorada no último mês fechado (mês sem compra = 0) dá **−3,5%**. 📌 O ritmo (horizonte curto) pode ser NEGATIVO com o ano positivo (+22,2%) — são horizontes diferentes, não é bug (dúvida do Diego, respondida).
- ⭐ **SIMULADOR "E se eu aumentar o volume"** (`_cpFornSimCard`/`_cpFornSimOutHTML`, `max-width:700px`): campo de %, **toggle A|B** do período base, e 3 blocos — **Volume · Valor · Ano cheio**. O bloco do ano projeta o **ANO SEGUINTE sobre o ano corrente projetado** (2027 sobre os R$ 1,48mi de 2026), não o ano da base. **Cada número tem a legenda em cima** (`5bc4e202`): rótulo cinza para o riscado (a base), rótulo no acento para o resultado, e `a mais` escrito após o delta. Datas/projeção ficam no `title`.
- ⭐ **ORDENAÇÃO CLICÁVEL em TODAS as tabelas de Compras** (`9ac06700`): motor único `_cpSort`/`_cpSortGet`/`_cpSortSet`/`_cpTh`/`_cpSortArr` (localeCompare pt-BR `numeric:true`; **vazio/null sempre no FIM**, nas duas direções). ⚠️ Em **Fornecedores** o rank/acumulado da **curva ABC é calculado na ordem de VALOR** e só depois as linhas são reordenadas p/ exibição — senão a curva mentiria. No **Histórico** a ordenação roda **ANTES** do teto de 250 linhas.
- **Ficha do ITEM** (`29e2a2a3`, `6ae7d1d7`, `0eca46a5`): **Cobertura (físico)** × **Cobertura (físico + OC)** no card Indicadores, com o período junto do preço médio e do custo médio; **volume nas barras** do gráfico de consumo; cronograma do simulador mostra "estoque atual dura até" **e** "com a OC em aberto dura até"; card de **OC em aberto compacto** (linha igual à de Últimas compras); resultado do simulador em **3 cards com título** (O que comprar · Preço e cobertura · Cronograma de chegada) com o cronograma em `.cp-crono-row` (2 colunas + info na linha de baixo, `:empty` some).
- **Defaults:** a ficha abre sempre com **A = mesmo período do ano passado** e **B = este ano** (`e1a01c23`).
- ⚠️ **ARMADILHA (já custou o app 1×):** **NUNCA usar crase em comentário dentro do template literal do `_cpCSS()`** — fecha a string e quebra tudo (`SyntaxError: Unexpected token ':'`). ⚠️ O Browser pane é **headless**: `window.innerWidth`=0, então `min(100%,760px)` colapsa p/ 0 na medição por JS (o screenshot renderiza certo) — usar `max-width` absoluto quando precisar medir.
- ⏳ **Em aberto (levantado, não pedido):** o simulador de compra da ficha do ITEM **ainda não desconta a OC em aberto** da quantidade sugerida; e as 8 sugestões da auditoria da entrada (b) seguem na fila.
...
- **Mapeamento (sheet `OrdensCompra`, 23 col):** `_CP_OCABERTO` `{cod,nome,um,qtd,valor,forn,prazo,emissao,prevista,oc}` — cod/nome=**Item(9)**, forn=**Fornecedor(3)**, um=**Unid Estoque(18)**, **qtd = Saldo a Entregar Estoque(23)** (o que vem → `_cpOcSaldo`), **valor = saldo × Valor Unitário Estoque(13)** (só a parte aberta), emissao=**Data Emissão(1)**, prevista=**Data Previsão Entrega(5)**, oc="OC-"+**Ordem(2)**, **prazo="" (o export NÃO tem prazo de pagamento** → coluna mostra "—"). `_CP_OC` (map cod→[]) `{oc,forn,emissao,prevista,qtd,entregue}` — **qtd=Qtde Estoque(17)**, **entregue=Qtde Entregue Estoque(21)** (saldo=qtd−entregue). Viraram **`JSON.parse`** (eram literais). Scripts `scratchpad/gen_oc.ps1` + `inject_oc.ps1` (regex `(?s)const _CP_OC = \{.*?\n\};`).
- ⚠️ **11 das 109 OCs são de itens que a heurística `_cpCategoria` põe em "Outros"** (Gema Pasteurizada, Fubá/Fecomix, Milho para Pipoca, Mignon Granel, Pellet Fritura, Melhorador de Massa) — são ALIMENTOS legítimos mal-classificados. **NÃO escopei a aba Ordens** (mostra as 109 reais), então elas aparecem em Ordens mas os ITENS não aparecem em Planejamento/Insumos (escopados fora como "Outros"). Se o Diego quiser consertar, é **adicionar esses termos às palavras-chave de `_cpCategoria` (~8871)**. `_cpOcSaldo` é keyed por cod e só é consultado por itens já no escopo — então funciona.
- ⭐ **FILTRO de OC morta (`5819595e`, mesmo dia):** o Diego pediu p/ tirar OCs "mais velhas que uma semana atrás". **`_cpOcViva(p)` = `!p.prevista || p.prevista >= (hoje−7)`** (dinâmico, `_cpOcCut()`), aplicado em **`_cpOcSaldo`** (disponível), **aba Ordens** (`_cpOrdRowsHTML` + `_comprasOrdensHTML`) e **ficha** (`_cpOCsItem`). Hoje=01/09 → corte **25/08**; das 109 OCs ficam **46 vivas** (63 removidas), "Entrega atrasada" 68→5. **Auto-ajusta** conforme o dia avança. ⚠️ O Diego disse "21/08 pra trás" (≈ hoje−11); usei a REGRA "uma semana atrás" (hoje−7 = 25/08). Se ele quiser exatamente 21/08, é trocar o `-7` por `-11` (ou data fixa) em `_cpOcCut`.
- ⚠️ (antes do filtro) as 109 OCs incluíam 68 "atrasadas" (previsão < hoje) — pedidos ANTIGOS (abril/etc.) com saldo residual no ERP. O filtro acima resolve isso. ⚠️ `_CP_CONS_25` (2025) e o resto seguem intactos. index.html 4,27→**4,30 MB**.

### 2026-08-31 (i) — PC da Empresa — Compras: ESCOPO do comprador (só Matéria-prima + Embalagens em todas as abas)
Pedido do Diego: "Manutenção tem outro comprador" → Compras deve mostrar **só Matéria-prima e Embalagens**. **Commit `ddef652f`, pushado; `main == origin`.** Verificado por DOM (:8899): 599 itens (301 MP + 298 Emb, era 1.900); Histórico 3.487→1.499 linhas (R$27,8mi→23,5mi); Fornecedores caiu p/ 105; sumiram Ventoinha/Cabo/Sensor/Chapa inox/BRINDEs; 0 erros.
- **Exclusões pontuais** (`9d9a4b8b`, o Diego apontou por print): `_cpEscComprador` também **exclui** nome que **começa com "mistura"** (misturas prontas), nome que contém **"industrializ"** ("Serviço de Industrialização - Moagem Gritz/Milho") e o **cód 200134** ("Cobertura de Chocolate Liquido - Meio Amargo"). Assinatura virou `_cpEscComprador(nome, cod)` (as 3 chamadas passam `l.item[0]`/`o.cod`). 599→**567 itens**. Pra tirar mais itens depois: é só adicionar `if(...)return false` nesse helper (~8618).
- **Helper `_cpEscComprador(nome, cod)`** = `_cpCategoria(nome) ∈ {Matéria-prima, Embalagens}` (+ exclusões acima). **`_cpItens()` filtra na ORIGEM** → cobre de uma vez Painel/Planejamento/Estoque/Insumos (todos usam `_cpItens()`). As views por LINHA (Histórico `_cpFiltradas`, Fornecedores `_cpFornAgg`) ganharam `if(!_cpEscComprador(l.item[1])) return`. **`_CP_CATS` reduzido a `['Matéria-prima','Embalagens']`** (chips de filtro). `_cpSimCriarTarefa` passou a usar `_cpIdx()[cod]` (não o `_cpItens()` agora filtrado). A **Necessidade da semana** já era escopada (seed de MP/embalagem).
- ⚠️ **A categoria é HEURÍSTICA por nome (`_cpCategoria`)** — imperfeita: (1) alguns **produtos acabados vazam pra "Embalagens"** pela palavra "pote" (ex.: "Pé de Moça (Pote 960g)"); (2) itens legítimos de MP/embalagem com nome atípico podem cair em "Outros" e **sumir**. Se o Diego notar item faltando ou errado, o ajuste é **refinar as palavras-chave de `_cpCategoria`** (~8871) OU, se ele preferir, trocar a regra p/ **excluir só Manutenção** (mantendo Outros/Limpeza) — decisão dele. O "Outros" tinha 921 itens (mistura de acabados + manutenção mal-classificada + misc), por isso "só MP+Emb" foi o corte escolhido.

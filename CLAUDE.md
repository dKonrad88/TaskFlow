...
- ⭐ **Colunas do Planejamento REORDENADAS + "Dura até" em 2 colunas** (`1f2239a6`→`36667f60`, pedido do Diego, várias iterações). Ordem FINAL (11 colunas, colspan do vazio=11): **Check · Cobertura · Dura até (físico) · Dura até (c/ OC) · Cód · Produto · Estoque físico · Estoque disponível · Média de consumo · Média do período · Tempo de entrega**. **"Dura até" desdobrado em 2** (`36667f60`): **(físico)** = `o.est/medPer` → data, cor pela faixa do próprio físico (`_cpPlanBandFor`); **(c/ OC)** = disponível (físico+OC) → data, cor da cobertura; **sem OC na linha → traço "—"** na coluna c/ OC. **Média de consumo e Média do período agora mostram a unidade (KG)** após o número (`cp-un`). Nova coluna **"Dura até"** (o Diego chamou de "estoque dura até"; encurtei o título) = a DATA em que o estoque acaba (`_cpDataFim(covDias)`), na cor da faixa de cobertura — fica **colada na Cobertura** (leem como par "2,6 meses → 17/11/26"). **Estoque físico** = só o saldo físico (limpo); **Estoque disponível** = físico + OC, com badge **"+N OC"** (a cobertura usa a disponível). O Diego pediu p/ manter as DUAS (tinha removido a disponível no meio do caminho e ele quis de volta — `be5e6358`).

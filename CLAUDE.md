...
- ⭐ **Colunas do Planejamento REORDENADAS + nova "Dura até"** (`1f2239a6`→`6084efb7`, pedido do Diego, várias iterações). Ordem FINAL: **Check · Cobertura · Dura até · Cód · Produto · Estoque físico · Média de consumo · Média do período · Tempo de entrega**. Nova coluna **"Dura até"** (o Diego chamou de "estoque dura até"; encurtei o título) = a DATA em que o estoque acaba (`_cpDataFim(covDias)`), na cor da faixa de cobertura — fica **colada na Cobertura** (leem como par "2,6 meses → 17/11/26"). **Removida a "Estoque disponível"**; o saldo em OC segue como badge **"+N OC"** no Estoque físico e **continua entrando na cobertura** (disp = est + OC).
- ⭐ **Alinhamento em 2 ZONAS** (`6084efb7`) — o Diego achou ruim o zigue-zague (coluna à direita, vizinha à esquerda). Agora: zona ESQUERDA (Cobertura · Dura até · Cód · Produto) toda `text-align:left`; bloco NUMÉRICO (os 4 à direita) toda `.cp-n`/`th.r`. Header bate com a célula em cada coluna. ⚠️ Se o Diego sentir falta da coluna "disponível" (est+OC explícito), é fácil repor — a info do OC virou o badge.

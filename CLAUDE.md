...
- **Removido o parágrafo explicativo** abaixo do título "Estoque" (pedido do Diego — a análise já sugeria encurtar; ele preferiu tirar). `_cpTela` já esconde o `<p>` quando `desc` é vazio → passei `''`. Topo vai direto título → chips → busca.
